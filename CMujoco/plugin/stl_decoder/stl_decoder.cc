...
// Explicit registration entry point. A SwiftPM static archive may drop a translation unit whose
// only externally-visible effect is a static constructor, so the binding references this symbol
// (via CShim's mj_registerBuiltinDecoders) to force the TU to be linked and to register the
// decoder deterministically. mjp_registerDecoder is dedup-safe (AppendIfUnique), so it is also
// invoked by the static constructor below where constructors do run.
extern "C" void mjregister_stl_decoder(void) {
  mjpDecoder decoder;
